...
def parse_args():
    ...
    parser.add_argument('-c', '--checkpoint', type=str, metavar='PATH', default='checkpoint/ModelNet40',
    ...
    for i in range(args.NUM_PEPEAT):
        ...
        for batch_idx, (data, label) in enumerate(testloader):
            ...
            for v in range(args.NUM_VOTE):
                ...
                with torch.no_grad():
                    pred += F.softmax(net(new_data.permute(0, 2, 1), 0.01)[0], dim=1)  # sum 10 preds
